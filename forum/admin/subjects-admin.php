...
//this page will display a list of subject that are under a room for an Admin
//the Admin will have all the contrll on the subjects
//he can delete close subjects

$room = new Room();
if (isset($_GET["room"]) && $_GET["room"]!="") {
	$room->setID($_GET["room"]);
	if($room->CheckRoom()==0)//checking if the room exists on the databse
...
<body>
	...
	?>
	<!--#######################################################################################################-->
	<div id="subject-room-top-div" style="margin-top:200px;">
			...
			//Loading more subjects
				...
				$("#load-more-subjects").click(function(){
					...
					$.ajax({
						type:"POST",
						url:"../api/api.php",
						data:{Room:Room,first:first,second:second,filter:filter,action:"SubjectListByRoomAdmin"},
